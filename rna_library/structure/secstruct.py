...
class SecStruct:
    ...
    def helix_replace_(self, h_id: int, secstruct: str, sequence: str) -> None:
        """
        Replaces the sequence and structure for a given Helix() object specified
        by h_id. SHOULD NOT be called by users directly.

        :param: int h_id: The counter_ -based index of the helix.
        :param: str secstruct: A dot bracket representation of the secondary
        structure replacement for the Helix().
    ...
    def motif_replace_(self, m_id: int, new_secstruct: str, new_sequence: str) -> None:
        """
        Replaces the sequence and structure for a given generic motif()
        object specified by m_id. SHOULD NOT be called by users directly.

        :param: int m_id: The counter_ -based index of the motif.
        :param: str secstruct: A dot bracket representation of the secondary
        structure replacement for the Motif().
        ...
        if motif.has_parent():
            ...
            # this is the case where you are actually just replacing the root?
            # seems like a dumb thing to do but who knows what people will try
            # these days
            pass
    ...
    def change_motif(
        ...
        """
        Client method for changing a Motif() object within a SecStruct() object.
        Currently, supports the changing of a Motif()'s sequence without changing
        the type/size of the motif as well as changing the Motif() itself and the
        child Motif() objects that it owns. For the case of Junction()/Hairpin()
        objects, the flanking pairs are not updated by default. This behavior
        can be changed via the include_flanking parameter. It is set to False
        by default but can be set to True to override the outermost respective
        closing pairs in the adjacent Helix() object(s).

        :param: int m_id : The id of the Motif() to change.
        :param: str new_secstruct: The new secondary structure to use, in
        dot-bracket format.
        :param: str new_sequence: The new sequence to use.
        :param: bool include_flanking: Whether the flanking pairs of the
        sequence for a Junction() or Hairpin() object should be overridden on
        their respective Helix() parent/children.
        ...
        # 3. more sophisticated breakout for changes in motif vs sequence vs
        # tree etcs
        assert len(new_sequence) == len(new_secstruct)
        assert len(re.sub("[AUCGT&]", "", new_sequence)) == 0
        assert len(re.sub("[.)(&]", "", new_secstruct)) == 0
        ...
        if helix_replace:
            if include_flanking and self.id_mapping_[m_id].parent.is_helix():
                self.id_mapping_[m_id].parent.change_inner_flanking(
    ...
    def get_ids(self, motif_type):
        ...
        for mtype, string in TYPE_MAPPER.items():
            ...
            print(
                f"{motif_type} is an invalid motif type. Allowed values "
    ...
    def get_num_motifs(self) -> int:
        return len(self.id_mapping_)
    ...
    def set_barcode(self, m_id, bc_seq):
        ...
        if not m.is_singlestrand() and not m.is_helix():
            raise TypeError(f"Barcode motif must be either a singlestrand or " f"helix")

        if not m.same_pattern(bc_seq):
            raise TypeError(
                f'The supplied barcode "{bc_seq}" is not compatible with '
                f"the selected motif"
            )
