...
class SecStruct:
    ...
    def change_motif_sequence(self, m_id: int, new_secstruct: str, new_sequence: str, include_flanking : bool = False) -> None:
    ...
    def change_motif(self, m_id: int, new_secstruct: str, new_sequence: str, include_flanking : bool = False) -> None:
        ...
        if helix_replace:
            if include_flanking and not self.id_mapping_[m_id].is_singlestrand():
                ...
                for s1, s2, c in zip(strands[:-1], strands[1:], self.id_mapping_[m_id].children()):
