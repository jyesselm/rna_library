...
class Motif(ABC):
    ...
    def link_children_(self, depth):
        ...
        self.children_ = [child for child in self.children() if not isinstance(child, list)]
    ...
    def str(self):
        ...
        if not self.has_children():
            ...
            for index, child in enumerate(self.children()):
                ...
                if index < len(self.children()) - 1 and \
                        self.children()[index+1].is_singlestrand():
    ...
    def __eq__(self, other):
        return self.type_ == other.type_ and \
                self.sequence_ == other.sequence_ and \
                self.token_ == other.token_
    ...
    def __init__(self, **kwargs):
        ...
        for index, nt in enumerate(self.sequence_):
            if nt == '&':
                ...
                self.structure_[index+1] = ')'
                self.closing_pairs_.append(self.sequence_[index-1] + self.sequence_[index+1]) 

        self.structure_[0] = '('
        self.structure_[-1] = ')'

        self.structure_ = "".join(self.structure_)
        self.token_ = f"Junction{len(self.strands())}_" + '|'.join([str(len(strand)-2) for strand in self.strands_])
...
def helix_length_(connections, start):
    ...
    length = 0
    while connections[start + length] == complement - length and \
            connections[complement - length] == start + length:
        length += 1
    return length 
...
def get_helix_(sequence, connections, start):
    ...
    if connections[start+helix_len-1] > start:
        helix.children_.append(get_junction_or_hairpin_(sequence, connections, start+helix_len-1))
...
class SecStruct:
    ...
    def change_motif(self, id, new_secstruct, new_sequence):
        ...
        assert len(re.sub('[AUCG&]', '', new_sequence)) == 0
        assert len(re.sub('[.)(&]', '', new_secstruct)) == 0
        assert new_secstruct.count('&') == new_sequence.count('&') and new_sequence.count('&') <= 1 
        ...
        full_secstruct, full_sequence = self.root_.recursive_structure(), self.root_.recursive_sequence()
